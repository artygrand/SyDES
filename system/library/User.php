...
class User{
	...
	public function login(){
		if (strlen($this->post['username']) < 20 && strlen($this->post['password']) < 20){
			if ($this->post['username'] == $this->username && md5($this->post['password']) == $this->password){
				...
				if (isset($this->post['remember']) && $this->autologin == 1){
	...
	public function isLoggedIn(){
		...
		if (isset($_SESSION['user'], $_SESSION['hash'])){
			if ($_SESSION['user'] == $this->username && $_SESSION['hash'] == md5($this->password . getip())){
		...
		// login by cookies
		} elseif ($this->autologin == 1 && isset($_COOKIE['user'], $_COOKIE['hash'])){
			if ($_COOKIE['user'] == $this->username && $_COOKIE['hash'] == md5($this->password . getip())){
	...
	public function hasToken(){
		return (isset($this->post['token']) && $this->post['token'] == $_SESSION['token']);
	}

	public function isAdmin(){
		if (isset($this->post['mastercode']) && md5($this->post['mastercode']) == $this->mastercode){
